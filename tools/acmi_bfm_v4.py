...
# ---------------------------------------------------------------------
# Vertical performance / "vertical UFO" helpers
# ---------------------------------------------------------------------
# These metrics are intended to catch the common SFM "infinite energy in the vertical"
# behavior that is hard to detect with turn-rate-only envelope checks.
#
# Notes:
# - All times are seconds *since the start of the object's track*.
# - Climb rates are computed as an average over a trailing time window (5s / 10s)
#   using linear interpolation to reduce sampling noise.

ALT_30K_FT = 30000.0
ALT_40K_FT = 40000.0
EARLY_WINDOW_S = 240.0

# Simple default thresholds for a "vertical UFO" flag.
# These are conservative starting points and can be tuned using a control run (e.g. F-86).
VERTICAL_UFO_CLIMB_ALT_FT_THRESHOLD = 20000.0
VERTICAL_UFO_CLIMB_FPM_THRESHOLD = 18000.0
VERTICAL_UFO_TIME_TO_40K_THRESHOLD_S = 300.0


def time_to_altitude_s(times: List[float], alt_ft: List[float], threshold_ft: float) -> Optional[float]:
    """Return seconds since track start when altitude first reaches threshold_ft.

    Uses linear interpolation between samples for a more precise crossing time.
    Returns None if never reached.
    """
    if not times or not alt_ft or len(times) != len(alt_ft):
        return None
    t0 = times[0]
    for i in range(len(times)):
        if alt_ft[i] >= threshold_ft:
            if i == 0:
                return 0.0
            a0 = alt_ft[i - 1]
            a1 = alt_ft[i]
            t_prev = times[i - 1]
            t_cur = times[i]
            if a1 == a0 or t_cur == t_prev:
                return times[i] - t0
            frac = (threshold_ft - a0) / (a1 - a0)
...
def time_at_max_altitude_s(times: List[float], alt_ft: List[float]) -> Optional[float]:
    """Return seconds since track start when max altitude occurs (first occurrence)."""
    if not times or not alt_ft or len(times) != len(alt_ft):
        return None
    t0 = times[0]
    max_alt = None
...
def max_alt_in_first_window(times: List[float], alt_ft: List[float], window_s: float) -> Optional[float]:
    """Max altitude (ft) within the first window_s seconds of the track."""
    if not times or not alt_ft or len(times) != len(alt_ft):
        return None
    t0 = times[0]
    max_alt = None
    for t, a in zip(times, alt_ft):
        if (t - t0) > window_s:
            break
        if (max_alt is None) or (a > max_alt):
            max_alt = a
...
def max_climb_fpm_over_window(
    times: List[float],
    alt_ft: List[float],
    window_s: float,
    *,
    max_rel_time_s: Optional[float] = None,
    min_alt_end_ft: Optional[float] = None,
) -> Tuple[Optional[float], Optional[float], Optional[float]]:
    """Compute max average climb rate over a trailing time window.

    Returns (max_climb_fpm, time_at_max_s, alt_at_max_ft), where time_at_max_s
    is seconds since track start.

    - max_rel_time_s: if set, only consider samples with t_rel <= this value.
    - min_alt_end_ft: if set, only consider windows where the end altitude >= this.
    """
    if not times or not alt_ft or len(times) != len(alt_ft):
        return (None, None, None)
    ...
    for i in range(n):
        t_rel = times[i] - t0
        if max_rel_time_s is not None and t_rel > max_rel_time_s:
            break

        t_start = times[i] - window_s
        if t_start < times[0]:
            continue

        # advance j so that times[j] is the first sample >= t_start
        while j < i and times[j] < t_start:
            j += 1

        if j == 0:
            continue

        # bracket is (j-1, j) with times[j-1] <= t_start <= times[j]
        t_a = times[j - 1]
        t_b = times[j]
        a_a = alt_ft[j - 1]
        a_b = alt_ft[j]
        ...
        if t_b == t_a:
            ...
            frac = (t_start - t_a) / (t_b - t_a)
            alt_start = a_a + frac * (a_b - a_a)

        alt_end = alt_ft[i]
        if min_alt_end_ft is not None and alt_end < min_alt_end_ft:
            continue

        rate_fpm = (alt_end - alt_start) * 60.0 / window_s
        if (best_rate is None) or (rate_fpm > best_rate):
            best_rate = rate_fpm
            best_t_rel = t_rel
            best_alt = alt_end

    return (best_rate, best_t_rel, best_alt)


def classify_vertical_ufo(
    *,
    max_climb_fpm_5s_above_alt: Optional[float],
    time_to_40k_s: Optional[float],
) -> Tuple[bool, List[str]]:
    """Return (flag, reasons[]) for vertical UFO heuristics."""
    reasons: List[str] = []

    if (
        max_climb_fpm_5s_above_alt is not None
        and max_climb_fpm_5s_above_alt > VERTICAL_UFO_CLIMB_FPM_THRESHOLD
    ):
        reasons.append(
            f"max_climb_fpm_5s_above_{VERTICAL_UFO_CLIMB_ALT_FT_THRESHOLD:.0f}ft="
            f"{max_climb_fpm_5s_above_alt:.0f} > {VERTICAL_UFO_CLIMB_FPM_THRESHOLD:.0f}"
        )

    if (
        time_to_40k_s is not None
        and time_to_40k_s < VERTICAL_UFO_TIME_TO_40K_THRESHOLD_S
    ):
        reasons.append(
            f"reached_40k_in={time_to_40k_s:.1f}s < {VERTICAL_UFO_TIME_TO_40K_THRESHOLD_S:.0f}s"
        )
...
def compute_flight_metrics(
    ...
    time_to_30k_s = time_to_altitude_s(times, alt_ft, ALT_30K_FT)
    time_to_40k_s = time_to_altitude_s(times, alt_ft, ALT_40K_FT)
    time_at_max_alt_s = time_at_max_altitude_s(times, alt_ft)

    max_climb_fpm_5s, _t_max_climb_5s, _alt_at_max_climb_5s = max_climb_fpm_over_window(
        times, alt_ft, 5.0
    )
    max_climb_fpm_10s, _t_max_climb_10s, _alt_at_max_climb_10s = max_climb_fpm_over_window(
        times, alt_ft, 10.0
    )

    max_alt_first_240s_ft = max_alt_in_first_window(times, alt_ft, EARLY_WINDOW_S)
    max_climb_fpm_5s_first_240s, _t_early, _alt_early = max_climb_fpm_over_window(
        times, alt_ft, 5.0, max_rel_time_s=EARLY_WINDOW_S
    )

    max_climb_fpm_5s_above_20k, _t_above, _alt_above = max_climb_fpm_over_window(
        times,
        alt_ft,
        5.0,
        min_alt_end_ft=VERTICAL_UFO_CLIMB_ALT_FT_THRESHOLD,
    )

    vertical_ufo, vertical_ufo_reasons = classify_vertical_ufo(
        max_climb_fpm_5s_above_alt=max_climb_fpm_5s_above_20k,
        time_to_40k_s=time_to_40k_s,
    )
    ...
    return {
        ...
        "time_to_30k_s": time_to_30k_s,
        "time_to_40k_s": time_to_40k_s,
        "time_at_max_alt_s": time_at_max_alt_s,
        "max_climb_fpm_5s": max_climb_fpm_5s,
        "max_climb_fpm_10s": max_climb_fpm_10s,
        "max_alt_first_240s_ft": max_alt_first_240s_ft,
        "max_climb_fpm_5s_first_240s": max_climb_fpm_5s_first_240s,
        "vertical_ufo": vertical_ufo,
        "vertical_ufo_reasons": vertical_ufo_reasons,
        # Peak turn metrics
...
def main():
    ...
    print()
    print("Vertical UFO thresholds (heuristics):")
    print(
        f"  max_climb_fpm_5s > {VERTICAL_UFO_CLIMB_FPM_THRESHOLD:.0f} fpm above {VERTICAL_UFO_CLIMB_ALT_FT_THRESHOLD:.0f} ft "
        f"OR reaches {ALT_40K_FT:.0f} ft within < {VERTICAL_UFO_TIME_TO_40K_THRESHOLD_S:.0f} s"
    )
    print(f"  Early window for vertical metrics: first {EARLY_WINDOW_S:.0f} s")
    print()
    ...
    for rec in results:
        ...
        def _fmt_opt(x, fmt="{:.1f}"):
            return "N/A" if x is None else fmt.format(x)

        print(
            f"  Vertical: t->30k={_fmt_opt(rec.get('time_to_30k_s'))} s, "
            f"t->40k={_fmt_opt(rec.get('time_to_40k_s'))} s, "
            f"max_alt={rec['max_alt_ft']:.0f} ft @ +{_fmt_opt(rec.get('time_at_max_alt_s'))} s"
        )
        print(
            f"  Climb: max_5s={_fmt_opt(rec.get('max_climb_fpm_5s'), '{:.0f}')} fpm, "
            f"max_10s={_fmt_opt(rec.get('max_climb_fpm_10s'), '{:.0f}')} fpm; "
            f"early_max_alt(0-{EARLY_WINDOW_S:.0f}s)={_fmt_opt(rec.get('max_alt_first_240s_ft'), '{:.0f}')} ft, "
            f"early_max_climb_5s={_fmt_opt(rec.get('max_climb_fpm_5s_first_240s'), '{:.0f}')} fpm"
        )
        print(
            f"  Vertical UFO: {'YES' if rec.get('vertical_ufo') else 'NO'} "
            f"reasons={'; '.join(rec.get('vertical_ufo_reasons') or []) if rec.get('vertical_ufo_reasons') else '[]'}"
        )
    ...
    if args.csv_out:
        fieldnames = [
            ...
            "max_climb_fpm_5s",
            "max_climb_fpm_10s",
            "max_alt_first_240s_ft",
            "max_climb_fpm_5s_first_240s",
            "vertical_ufo",
            "vertical_ufo_reasons",
            "max_inst_tr_deg_s",
            "speed_at_max_tr_kt",
            "alt_at_max_tr_ft",
            "g_at_max_tr",
        ...
        with open(args.csv_out, "w", newline="", encoding="utf-8") as f:
            ...
            for rec in results:
                ...
                row["vertical_ufo_reasons"] = ";".join(rec.get("vertical_ufo_reasons", []))
